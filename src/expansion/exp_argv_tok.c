...
/*   Updated: 2025/12/04 14:39:24 by joloo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "expansion_argv_internal.h"

int	expansion_argv_tok(t_exp_argv_data *data)
{
	int	i;
	...
	while(data->argv[i] != NULL)
	{
		data->in_squote = 0;
		data->in_dquote = 0;
		if (expansion_argv_tok_process(data, data->argv[i]) == FAILURE)
			return (FAILURE);
...
int	expansion_argv_tok_process(t_exp_argv_data *data, char *str)
{
	int	j;
